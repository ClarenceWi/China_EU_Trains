...
def extract_declaration_info(text, index):
    ...
    # 4. 提取贸易国（启运国）
    match = re.search(r'启运国（地区）\(\w+\)\s*(.*?)\s*经停港', text, re.DOTALL)
    if match:
        info['贸易国'] = match.group(1).strip()
    
    # 5. 提取主要商品名称和金额
    # 匹配商品信息行，金额后跟着两个"(XXX)"格式的字段
    pattern = r'项号\s+商品编号\s+商品名称及规格型号\s+数量及单位\s+单价/总价/币制.*?\s+\d+\s+(\d+)(\D+?)\s+\d+\.\d+\s+.*?(\d+\.\d+)\s+\(\w{3}\)\s+\(\w{3}\)'
    ...
    if match:
        ...
        try:
            ...
            info['折合人民币金额'] = round(info['金额'] * info['汇率'], 2)
        except ValueError:
            pass
...
def get_valid_directory():
    ...
    print("\n请选择PDF文件所在的目录:")
    while True:
        pdf_dir = input("请输入目录路径 (例如: C:\\pdf_files 或 D:\\documents): ").strip()
...
def main():
    ...
    # 获取输出Excel路径
    default_output = os.path.join(os.path.expanduser('~'), 'Desktop', '报关单汇总结果.xlsx')
